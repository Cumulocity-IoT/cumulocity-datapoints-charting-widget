...
export class CumulocityDataPointsChartingWidget implements OnInit, OnDestroy {
  ...
  verifyConfig(): boolean {
    //optimism
    this.widgetHelper.getChartConfig().enabled =
    ...
    if (this.widgetHelper.getChartConfig().enabled) {
      if (
        !this.widgetHelper.getWidgetConfig().selectedDevices.length ||
        !this.widgetHelper.getWidgetConfig().selectedMeasurements.length
      ) {
        //1: do we have devices
        this.widgetHelper.getChartConfig().enabled = false;
        this.widgetHelper.getChartConfig().message =
          "You must choose at least one device and fragment to plot a chart.";
        ...
        if (this.widgetHelper.getChartConfig().getChartType() == "bubble") {
          if (checks.series != 3 || !checks.x || !checks.y || !checks.r) {
            this.widgetHelper.getChartConfig().enabled = false;
            this.widgetHelper.getChartConfig().message =
              "You must choose exactly 3 fragments and assign x,y, and r.";
          }
        } else if (checks.series != 2) {
          this.widgetHelper.getChartConfig().enabled = false;
          this.widgetHelper.getChartConfig().message =
            "You must choose exactly 2 fragments and assign x,y.";
        ...
        this.widgetHelper.getChartConfig().enabled = false;
        this.widgetHelper.getChartConfig().message =
          "There appears to be no data selected to plot a chart (check series).";
      ...
      if (_.has(frag, options.series)) {
        ...
        if (options.groupby && newPointBucket === lastPointBucket) {
          ...
          if (
            ...
            let v: any = this.seriesData[key].valtimes[
              this.seriesData[key].valtimes.length - 1
            ].x;
            this.seriesData[key].valtimes[
              this.seriesData[key].valtimes.length - 1
            ].x =
              (<number>datum.x + v * (this.seriesData[key].valCount - 1)) /
              this.seriesData[key].valCount;
            this.seriesData[key].valtimes[
              this.seriesData[key].valtimes.length - 1
            ].x = parseFloat(
              (<number>(
            ...
            this.seriesData[key].valtimes[
              ...
              (<number>datum.y + v * (this.seriesData[key].valCount - 1)) /
              this.seriesData[key].valCount;
            this.seriesData[key].valtimes[
              this.seriesData[key].valtimes.length - 1
            ].y = parseFloat(
        ...
        if (
          ...
          if (options.avgPeriod > 0) {
            ...
            let source = this.seriesData[key].valtimes
              ...
              .map((val) =>
                options.targetGraphType !== "horizontalBar" ? val.y : val.x
              );
        ...
        if (this.chartElement) {
          ...
          if (
            ...
            if (this.widgetHelper.getChartConfig().aggregation == 0) {
              ...
              if (this.widgetHelper.getChartConfig().customFormat) {
                aggFormat = this.widgetHelper.getChartConfig()
                  .customFormatString;
              }
          ...
          if (
            this.widgetHelper.getChartConfig().getChartType() === "line" ||
            this.widgetHelper.getChartConfig().getChartType() === "spline" ||
            (!this.widgetHelper.getChartConfig().multivariateplot &&
   ...
   */
  private createMultivariateChart(
    seriesList: { [id: string]: string },
    localChartData: any[]
  ) {
    let seriesName = this.widgetHelper.getChartConfig().series[seriesList["x"]]
      .name;
    if ("y" in seriesList) {
      seriesName =
        seriesName +
    ...
    if (!("timer" in this.subscription)) {
      ...
      this.subscription["timer"] = setInterval(
        this.handleTimer,
        this.widgetHelper.getChartConfig().timerDelay * 1000,
        this
      );
    }
  }
  ...
  private createPieChart(
    ...
    if (!this.subscription[key]) {
      this.subscription[key] = this.realtimeService.subscribe(
  ...
  private createNormalChart(
    ...
    if (
      this.widgetHelper.getChartConfig().series[key].hideMeasurements !== true
    ) {
      let thisSeries: ChartDataSets = this.createSeries(
        key,
